package com.limaoso.phonevideo.play.utils;
...
public class SDUtils {
	...
	public  static long getTotalAvailableSize(String path) {
			try {
				...
				StatFs stat = new StatFs(base.getPath());
				 long nSDTotalSize =stat.getBlockSize()*((long) stat.getBlockCount());
				return nSDTotalSize;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static long getCacheAvailableSize(){
		return getAvailableSize(getRootFile());
	}
	/**
	 * 获取缓存空间总大小
	 * @return
	 */
	public static long getCacheTotalAvailableSize(){
		return getTotalAvailableSize(getRootFile());
	}
	/**
	 * 获取 /data/data/package.name/   目录
	 * @param path data目录文件夹名
	 * @return
	 */
	public static String getRootDataPath(String pathName){
		String infoRootPath = SDUtils.getInternalMemoryPath()
				+ File.separator + "data" + File.separator
				+ UIUtils.getContext().getPackageName() + File.separator + pathName
				+ File.separator;
	 ...
	 */
	//p2p/download
	public static String getDownFileDir(){
		String path = getRootFile()+File.separator+"p2p"+File.separator+"download"+File.separator;
		File file = new File(path);
		if (!file.exists()) {
			file.mkdirs();
		}
	...
	public static String getRootFile() {
		...
		String rootPath= PrefUtils.getString(UIUtils.getContext(), flagRootPath, null);
		if (rootPath!=null) {
		...
		String sdPath = getSDPath() + File.separator + GlobalConstant.ROOT_DIR_NAME+File.separator;
		if (sdPath != null) {
			...
			PrefUtils.setString(UIUtils.getContext(), flagRootPath, file.getAbsolutePath().toString());
			return file.getAbsolutePath().toString();
		}
		return null;
	}
