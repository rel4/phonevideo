...
import com.limaoso.phonevideo.play.adapter.UserCommentAdapter;
...
public class PlayCommentActivity extends BaseActivity implements
	...
	protected void initNet() {
		mHttphelp.sendGet(NetworkConfig.getCommentURL(mId, current_tag, mPage),
				VideoCommentBean.class,
				new MyRequestCallBack<VideoCommentBean>() {
					...
					public void onSucceed(VideoCommentBean bean) {
						...
						if ("1".equals(bean.status)) {
							...
							if (A_Tag.equals(current_tag)) {
								...
								mTv_comment_user_name
										.setText(bean.cont.mainComment.nickname);
								mTv_comment_user_content
										.setText(bean.cont.mainComment.msg);
								mTv_comment_user_time
										.setText(bean.cont.mainComment.time);
								mHttphelp.showImage(mCiv_comment_user_icon,
										bean.cont.mainComment.face);
								mContentAdapter = new UserCommentAdapter(
										mActivity, bean.cont.commentList,
										R.layout.item_comment_layout);
								mLv_content.setAdapter(mContentAdapter);
								mPage++;
							} else {
								if (bean.cont.commentList != null
										&& bean.cont.commentList.size() > 0) {
									mPage++;
	...
	protected void onDestroy() {
		...
		// EventBus.getDefault().post(new PlayVideo_event(update_num_tag,
		// null));
		super.onDestroy();
	}
