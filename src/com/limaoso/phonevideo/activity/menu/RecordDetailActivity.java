...
import com.limaoso.phonevideo.bean.RecordPageBean.List_all;
...
public class RecordDetailActivity extends BaseActivity implements
	...
	private void initPageData() {
		...
		mHttpHelp.sendGet(NetworkConfig.getRecordPage(), RecordPageBean.class,
	...
	private class RecordAadater extends BaseAdapter {
		...
		@Override
		public int getCount() {
			return recordTodayNames.size() + recordWeekNames.size()
					+ recordOldNames.size() + 3;
		}
		...
		private void isShowContentFrom(int position, AllViewholder vhc) {
			 ...
			 */
			if (position > 0 && position < recordTodayNames.size() + 1) {

				vhc.recordTitle.setText(recordTodayNames.get(position_t));
				vhc.recordTime.setText(recordTodayTime.get(position_t));
				mHttpHelp.showImage(vhc.recordIcon,
						recordTodayIcon.get(position_t));

			} else if (position >= recordTodayNames.size() + 2
					&& position < recordTodayNames.size()
							+ recordWeekNames.size() + 2) {

				vhc.recordTitle.setText(recordWeekNames.get(position_w));
				vhc.recordTime.setText(recordWeekTime.get(position_w));
				mHttpHelp.showImage(vhc.recordIcon,
						recordWeekIcon.get(position_w));
					...
					&& position < recordTodayNames.size()
							...
							+ 3) {
				vhc.recordTitle.setText(recordOldNames.get(position_o));
				vhc.recordTime.setText(recordOldTime.get(position_o));
				mHttpHelp.showImage(vhc.recordIcon,
						recordOldIcon.get(position_o));
			}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		int weekScope = 1 + recordTodayID.size() + 1 + recordWeekID.size();// 一周内的范围内
	...
	public void executeAllDeleteOperate() {
		if (recordTodayIcon.size() + recordWeekIcon.size()
				+ recordOldIcon.size() > 0) {
	...
	public void onLoadMore() {
		...
		if (isInitLoad) {
			lUrl = mbean.url + "&ucode=" + NetworkConfig.getUcode() + "";
			tempUrl = lUrl;
			isInitLoad = false;
		} else {
		...
		mHttpHelp.sendGet(lUrl, RecordPageBean.class,
				new MyRequestCallBack<RecordPageBean>() {
					...
					public void onSucceed(RecordPageBean bean) {
						...
						initNetworkData(false, bean); // 上拉加载的时候再次设置网络数据
